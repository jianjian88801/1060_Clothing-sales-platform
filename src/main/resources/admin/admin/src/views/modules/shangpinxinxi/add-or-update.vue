<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="颜色" prop="yanse">
          <el-select v-model="ruleForm.yanse" placeholder="请选择颜色">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="尺码" prop="chima">
          <el-select v-model="ruleForm.chima" placeholder="请选择尺码">
            <el-option
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yanse/yanse`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yanseOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../springboot4r3y8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
